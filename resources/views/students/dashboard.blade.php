    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        ...
                        @foreach($notifications as $note)
                            <div class="bg-white border border-gray-100 p-4 rounded-2xl shadow-sm flex items-start space-x-4">
                                <div class="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center 
                                    {{ $note['color'] == 'red' ? 'bg-red-50 text-red-600' : '' }}
                                    {{ $note['color'] == 'blue' ? 'bg-blue-50 text-blue-600' : '' }}
                                    {{ $note['color'] == 'amber' ? 'bg-amber-50 text-amber-600' : '' }}">
                                    
                                    @if($note['type'] == 'deadline')
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    @elseif($note['type'] == 'new')
            ...
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-lg font-bold text-gray-700 uppercase tracking-wider flex items-center">
                    <svg class="w-5 h-5 mr-2 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                    </svg>
                    My Classroom
                </h3>
                <a href="{{ route('student.enroll.index') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-xs font-bold uppercase tracking-widest rounded-xl hover:bg-blue-700 shadow-sm transition-all duration-200">
